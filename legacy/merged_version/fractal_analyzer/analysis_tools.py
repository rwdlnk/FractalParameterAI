...
class FractalAnalysisTools:
    ...
    def analyze_linear_region(self, segments=None, box_sizes=None, box_counts=None, bounding_box=None, 
                             fractal_type=None, plot_results=True,
                             plot_boxes=True, trim_boundary=0, save_plots=False,
                             output_dir=None, custom_prefix=None, sigma_thresh=0.05, L_min=5):
        """
        ...
        segments : list, optional
            List of segments to analyze (required if box_sizes and box_counts not provided)
        box_sizes : array, optional
            Pre-computed box sizes (if None, computed from segments)
        box_counts : array, optional
            Pre-computed box counts (if None, computed from segments)
        bounding_box : tuple, optional
            Pre-computed bounding box (min_x, min_y, max_x, max_y)
        ...
        sigma_thresh : float, default=0.05
            Threshold for standard deviation of slopes to identify stable regions
        L_min : int, default=5
            Minimum length for candidate regions
        ...
        # If box counting data not provided, compute it from segments
        if box_sizes is None or box_counts is None:
            if segments is None:
                raise ValueError("Either segments or box counting data must be provided")
            ...
            print("Using provided box counting data")
            if bounding_box is None and segments is not None:
                # Calculate bounding box from segments if not provided
                min_x = min(min(s[0][0], s[1][0]) for s in segments)
                max_x = max(max(s[0][0], s[1][0]) for s in segments)
                min_y = min(min(s[0][1], s[1][1]) for s in segments)
                max_y = max(max(s[0][1], s[1][1]) for s in segments)
                ...
                bounding_box = (min_x - margin, min_y - margin, max_x + margin, max_y + margin)
